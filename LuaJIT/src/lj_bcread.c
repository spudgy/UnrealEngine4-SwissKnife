...
/* new vars for shuffling of opcodes */
unsigned int bc_translation_started = 0;
unsigned int bc_internal_ops[BC__MAX];
...
#define STR_XOR uint8_t bXor = 0xAA; \
...
static void bcread_kgc(LexState *ls, GCproto *pt, MSize sizekgc)
{
  ...
  for (i = 0; i < sizekgc; i++, kr++) {
    ...
    if (tp >= BCDUMP_KGC_STR) {
      ...
      memcpy(pCopy, p, len);
      STR_XOR
      setgcref(*kr, obj2gco(lj_str_new(ls->L, pCopy, len)));
      free(pCopy);
...
static void bcread_bytecode(LexState *ls, GCproto *pt, MSize sizebc)
{
  ...
  /* translate opcodes */
  if(bc_translation_started) {
    int read_opcode = bc_op(bc[1]);
    int translated_opcode = bc_internal_ops[read_opcode];

    setbc_op(bc + 1, translated_opcode);
  }
}
...
static int bcread_header(LexState *ls)
{
  ...
  // check random byte code garbage length
  MSize obfuscateByteLength = bcread_byte(ls);
  ls->p += obfuscateByteLength;
...
{
  /* construct our opcode array*/
  #define BCINTERNALOPCODES(name, ma, mb, mc, mt, opcode)    bc_internal_ops[opcode] = BC_##name;
  BCDEF(BCINTERNALOPCODES)
  #undef BCINTERNALOPCODES
  bc_translation_started = 1;
  ...
  for (;;) {  /* Process all prototypes in the bytecode dump. */
    ...
    uint8_t bXor = 0xAA;
    for (uint32_t i = 0; i < len;i++) {
      *(uint8_t*)&startp[i] = startp[i] ^ bXor--;
    }
