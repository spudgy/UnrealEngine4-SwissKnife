...
/* array of opcodes to allow translation to shuffled opcodes */
unsigned int bc_ops[] = {
    #define BCENUM(name, ma, mb, mc, mt, opcode)    opcode,
    BCDEF(BCENUM)
    #undef BCENUM
};
...
static inline str_xor_inc(char* buf, size_t len) {
    uint8_t num = 0xAA;
...
static inline str_xor_dec(char* buf, size_t len) {
    uint8_t num = 0xAA;
    for (uint32_t i = 0; i < len; i++) {
        buf[i] ^= num--;
    }
...
static void bcwrite_kgc(BCWriteCtx *ctx, GCproto *pt)
{
  ...
  for (i = 0; i < sizekgc; i++, kr++) {
    ...
    if (tp >= BCDUMP_KGC_STR) {
      MSize len = gco2str(o)->len;
      char* pCopy = (char*)malloc(len);
      memcpy(pCopy, strdata(gco2str(o)), len);
      str_xor_inc(pCopy, len);
      p = lj_buf_wmem(p, pCopy, len);
      free(pCopy);
...
#endif
  /* translate to shuffled opcodes */
  q[LJ_ENDIAN_SELECT(0, 3)] = bc_ops[(BCOp)q[LJ_ENDIAN_SELECT(0, 3)]];
  return p;
}
/* Write prototype. */
...
{
  // determine how long our bytecode garbage is
  unsigned char minObfuscateByteLength = 1;
  unsigned char maxObfuscateByteLength = 255;
  size_t obfuscateByteLength = rand() % (maxObfuscateByteLength - minObfuscateByteLength + 1) + minObfuscateByteLength;

  GCstr *chunkname = proto_chunkname(ctx->pt);
  const char *name = strdata(chunkname);
  MSize len = chunkname->len;
  char *p = lj_buf_need(&ctx->sb, (MSize)(3+5+5+len+1+obfuscateByteLength));
  *p++ = BCDUMP_HEAD1;
  *p++ = BCDUMP_HEAD2;
  *p++ = BCDUMP_HEAD3;
  *p++ = BCDUMP_HEAD4;
  ...
  *p++ = (ctx->strip ? BCDUMP_F_STRIP : 0) +
  ...
  // write length of bytecode garbage
  *p++ = (char)obfuscateByteLength;

  // generate random bytecode garbage
    size_t i;
  for (i = 0; i < obfuscateByteLength; i++) {
    *p++ = rand() % 256;
  }
